...
def init_db() -> None:
    with closing(get_db()) as db:
        ...
        UPDATE ai_requests SET status='failed',error=COALESCE(error,'Активный запрос прерван после перезапуска сервера'),completed_at=COALESCE(completed_at,CURRENT_TIMESTAMP) WHERE status IN ('queued','processing');
...
def mark_ai_request_processing(request_id:int,message_id:int|None=None):
    with closing(get_db()) as db:
        cur=db.execute(
            "UPDATE ai_requests SET status='processing',message_id=COALESCE(?,message_id),started_at=? "
...
def stream_request(payload:MessageRequest,request:Request,background_tasks:BackgroundTasks):
    ...
    with closing(get_db()) as db:
        ...
        try:
            ...
            raise
    try:
        mark_ai_request_processing(request_id,user_message_id)
    except RuntimeError:
        raise HTTPException(409,"Запрос уже остановлен")
    with closing(get_db()) as db:
        ai_messages=_chat_ai_messages(db,payload.chat_id)
